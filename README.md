...
What is still lost: the policy is the default `appendfsync everysec` (checked with `CONFIG GET`), so a power or VM failure can lose about a second of writes (the Redis documentation describes up to two when a flush is slow; not measured here), while a Redis process killed with SIGKILL straight after a reply lost nothing in 6 of 6 tries on a scratch volume, because the operating system still held the data. `appendfsync always` would close the gap at a cost on every write. And everything is lost on `docker compose down -v`. The 5-rejections-in-10-minutes throttle and the voiceprints are in Postgres, not Redis, and the spoof count behind a lock is rebuilt from Postgres, so a lost lock comes back at the next spoof detection only if two earlier ones are still inside the 30-minute window; otherwise the lock is gone and the attacker starts again with a fresh budget of three.
...
- **A take that was not accepted starts the screen over with a new phrase.** Verification already fetched a new challenge after every rejection (the server deletes a challenge the moment it judges it). Enrollment did not: after a `phrase_mismatch` or `spoof_detected` it kept the same phrase, left the words the last take had lit on screen, and left "Đổi câu" disabled, so the user was stuck with it. Now the page asks `/enroll/reshuffle` for another phrase (the server keeps the phrase it expects for each sample, so the two change together; if the swap is refused the old one stays valid), clears the lit words and re-enables the button. Four more things on the same path, all found by reproducing it: (1) if the refresh after a rejected verify take failed (the mic would not reopen, or `/verify/prompt` answered 429), the page kept the old phrase and hid the record button with no message, while the server had already thrown that phrase away; a mic that will not reopen no longer stops the new phrase from showing, and if no new phrase can be fetched the dead one is taken down and only "Đổi câu khác" is offered; (2) an error reply that is not a rejection (unreadable audio, 429, an expired enrollment, no connection, an HTML error page from nginx) used to advance enrollment to the next phrase as if the sample had been saved, leaving the screen a sample ahead of the server; it now stays on the same sample and shows the server's message; (3) the microphone the user picked fell back to the default one after every rejected verify take, a plausible source of "not heard", and is now kept; (4) recording first tries to reopen a mic that has gone away and says so if it cannot. Checked by running the real `frontend/app.js` against a scripted stub DOM: 64 checks, all passing, of which the previous version failed 39 (and one scenario of it died on an uncaught error). That is not a real browser or a real microphone, which I have not tried.
- The consent text and its version are tied together by a test. `CURRENT_CONSENT_VERSION` (`services/consent.py`) is what a user's consent record stores, while the words they read live in `frontend/index.html`, so nothing forced the two to change together: the text could be edited and everyone who had agreed to the old wording stayed "consented". `tests/test_consent_version.py` keeps a SHA-256 of the consent text for each version and fails if the text changes without a new version (which asks every user to agree again). It hashes the text a reader sees in that block (tags stripped, whitespace collapsed), so a changed class, re-indenting or reordering the markup does not trip it, but changing or reordering the wording does; the button labels and the strings in `app.js` are outside the block and not covered; and it cannot tell a bumped version from someone simply overwriting the expected hash of the current one, which review has to catch. It is a check run by the test suite (CI and a developer's machine), not one made when the backend starts: nginx serves `frontend/` from its own mount, which the backend container does not have (the test skips there), so editing `index.html` on a running stack without bumping the version and without running the tests would still not ask anyone to agree again. A consent row also stores only the version, not the text itself. Making it a runtime invariant would mean owning the text in the backend, serving it to the page and storing its hash with each consent (a migration); that is not done.
